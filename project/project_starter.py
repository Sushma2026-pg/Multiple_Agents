...
dotenv.load_dotev(dotenv_path=".env")
openai_api_key=os.getenv("UDACITY_API_KEY")


model = OpenAIServerModel(
    model_id="gpt-40-mini"
...
def get_all_inventory_tool(as_of_date: str) -> Dict[str, Dict[str, int]]:
    ...
    Use this tool to get a high-level snapshot of all items currenlty in stock as of a given date,
    including each item's minimum stock threshold for reorder decisions.

    Call this tool when:
    - You need to monitorr overall stock health and identify items that are below their reorder threshlod.
    - You need a global inventory view to decide whether any items should be reordered.

    Args:
        as_of_date (str): ISO-formated date string (YYYY-MM-DD) reprsenting the inventory cutoff.
...
def get_stock_level_tool(item_name: str, as_of_date: Union[str, datetime]) -> int:
    ...
    Call this tool when:
    - You have fuzzy-matched a customer's item request to an exact item in our catalog.
    - You need to confirm whether we have enough stock to fulfill the customer's requested quantity.
    -You need the stock count to pair historical pricing when building a quote.
...
def get_supplier_delivery_date_tool(input_date_str: str, quantity: int) -> str:
    ...
    Estimate the suppiler delivery date based on the requested order quantity and starting_date.
...
def generate_financial_report_tool(as_of_date: Union[str, datetime]) -> Dict:
    ...
    Returns:
        ...
        Dict: A dictionary containing the financial report fields:
            ...
            - 'inventory_value': Total value of inventory
            - 'total_assets': Combined cash and inventory value
            - 'inventory_summmary': List of items with stock and valuation details
            - 'top_selling_products':List of top 5 products by revenue
    """
...
def search_quote_history_tool(search_terms: List[str], limit: int = 5) -> List[Dict]:
    ...
    The function searches both original customer request (from `quote_requests`) and 
    the explanation for the quote (from `quotes`) for each keywords. Results are sorted by
    most recent order date and limited by the `limit` parameter.

    Args:
        search_terms (List[str]): List of terms to match against customer requests and explanations.
        limit (int, optional): Maximum number of quote records to return, Defauly is 5.

    Returns:
        List[Dict]: A list of matching quotes, each represented as a dictionary with feilds:
            - original_request
            - total_amount
            - quote_explanation
            - job_type
...
def get_cash_balance_tool(as_of_date: Union[str.datetime]) -> float:
    ...
    Args:
        as_of_date (str or datetime): The cutoff date (inclusive) inISO format or as a datetime object.
...
This funtion records a transaction if type 'stock_orders' or 'sales' with a specified item_name,
quantity, total price, and transaction date into the 'transactions' table of the database.

Args:
    item_name (str): The name of the item invlved in the transaction.
    transaction_type (str): Either 'stock_orders' or 'sales'
    quantity (int): Number of units involved in the transaction.
    price (float): Total price of the transaction.
    date (str or datetime): Date of the transaction in ISO 8601 format.
